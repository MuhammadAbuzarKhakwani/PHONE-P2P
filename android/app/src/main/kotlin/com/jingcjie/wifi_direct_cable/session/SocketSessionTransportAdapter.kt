...
private class SocketSessionTransport(
    ...
    /**
     * Null until the pairing exchange completes, at which point every subsequent
     * frame on this socket uses the v3 authenticated framing instead of v2.
     * Volatile because the reader and writer run on different threads.
     */
    @Volatile
    private var secureChannel: SecureChannel? = null

    override val isSecure: Boolean get() = secureChannel != null

    override fun activateSecurity(secureChannel: SecureChannel) {
        synchronized(writeLock) {
            check(this.secureChannel == null) {
                "security already activated on the ${channel.protocolName} channel"
            }
    ...
    override fun readFrame(): ProtocolFrame? {
        ...
        return if (secure != null) {
            secure.readFrame(socket.getInputStream())
        } else {
            ProtocolCodec.readFrame(socket.getInputStream())
        }
    }
    ...
    override fun writeFrame(frame: ProtocolFrame) {
        synchronized(writeLock) {
            val secure = secureChannel
            if (secure != null) {
                secure.writeFrame(frame, socket.getOutputStream())
            } else {
